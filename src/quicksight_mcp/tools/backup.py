...
def register_backup_tools(
    ...
    @mcp.tool
    def restore_from_backup(analysis_id: str, backup_file: str) -> dict:
        """Restore an analysis from a JSON backup file.

        Use this to recover from a FAILED analysis state or to undo
        changes. This is the only way to update a FAILED analysis
        (normal update_analysis refuses to modify FAILED analyses).

        A pre-restore backup is automatically created when possible.

        Args:
            analysis_id: The QuickSight analysis ID to restore.
            backup_file: Full path to the backup JSON file.
                Backups are saved to ~/.quicksight-mcp/backups/ by default.
        """
        start = time.time()
        client = get_client()
        try:
            client.restore_from_backup(backup_file, analysis_id)
            get_tracker().record_call(
                "restore_from_backup",
                {"analysis_id": analysis_id, "backup_file": backup_file},
                (time.time() - start) * 1000,
                True,
            )
            return {
                ...
                "note": (
                    "Analysis restored. Use describe_analysis to verify "
